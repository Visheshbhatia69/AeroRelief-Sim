"""Streamlit dashboard for running and inspecting AeroRelief-Sim."""
...
def sidebar_controls() -> tuple[SimulationConfig, int, int]:
    """Read the scenario, communication and PSO settings from the sidebar."""
...
def show_flight_status(config: SimulationConfig, swarm_size: int, iterations: int) -> None:
    """Show the main scenario settings above the results."""
...
def show_figure_note(title: str, text: str) -> None:
    """Add a short explanation under figures that need context."""
    st.markdown(
        f"""
...
def show_launch_sequence() -> None:
    """Display a small animated panel while the run is being prepared."""
    st.html(
        """
...
def show_mission_widgets(result: ExperimentResult) -> None:
    """Show the headline gains after a simulation run."""
...
def show_project_focus() -> None:
    """Show the project aim, baseline methods and objective formula."""
...
def save_dashboard_figures(result: ExperimentResult) -> dict[str, Path]:
    """Regenerate dashboard figures for the latest run."""
...
def plot_uav_movement_paths(result: ExperimentResult, output_path: Path) -> None:
    """Plot how UAV positions change from baseline to the other methods."""
...
def show_summary(result: ExperimentResult) -> None:
    """Show the four key result metrics at the top of the page."""
    baseline = result.baseline_metrics
    optimized = result.optimized_metrics
...
def show_major_findings(result: ExperimentResult) -> None:
    """Write the main findings from the current run in plain language."""
    baseline = result.baseline_metrics
...
def show_evidence_summary(result: ExperimentResult) -> None:
    """Explain what each dashboard result view is meant to support."""
    rows = [
        {
...
def show_result_story_chart(result: ExperimentResult) -> None:
    """Use line-style charts to show how the methods compare."""
...
def relative_cohesion_scores(component_counts: list[float]) -> list[float]:
    """Convert network-fragment counts into a higher-is-better display score."""
...
def show_objective_contribution_chart(result: ExperimentResult) -> None:
    """Break down the weighted objective score for each placement method."""
    rows = pd.DataFrame(
        [
...
def show_demand_relief_heatmap(result: ExperimentResult) -> None:
    """Show where demand was high before PSO and what remains uncovered."""
...
def demand_relief_grids(
    ...
    """Build demand grids from users, priority-zone weights and UAV coverage."""
    config = result.config
...
def covered_priority_zone_indices(
    ...
    """Find which priority zones are reached by the selected UAV placement."""
...
def show_metric_table(result: ExperimentResult) -> None:
    """Show the raw metric values used behind the charts."""
    methods = {
...
def show_objective_breakdown(result: ExperimentResult) -> None:
    """Show the exact objective-score terms used by the optimizer."""
...
def objective_rows(
    ...
    """Create table rows for the objective-score calculation."""
    config = result.config
    metrics = (
        result.baseline_metrics
...
def metric_row(
    ...
    """Format one dashboard metric row."""
    row = {
        "Metric": label,
        "Unit": unit,
    }
...
def format_metric_value(value: float, unit: str) -> str:
    """Format values with the unit style used in the table."""
    if unit in {"%", "weighted %"}:
        return f"{value:.2f}%"
...
def show_sensitivity_panel(
    ...
    """Run extra experiments by changing one input at a time."""
    st.markdown(
...
def run_selected_analysis(
    ...
    """Dispatch the selected sensitivity-analysis run."""
...
def show_selected_analysis_chart(analysis_type: str, df: pd.DataFrame) -> None:
    """Choose the chart that matches the generated analysis table."""
    if analysis_type == "Number of UAVs":
...
def run_repeated_experiments(
    ...
    """Repeat the same scenario with different random seeds."""
    rows = []
...
def flatten_result(run_number: int, result: ExperimentResult, scenario_type: str) -> dict:
    """Flatten one experiment result into a CSV-friendly row."""
    baseline = result.baseline_metrics
    kmeans = result.kmeans_metrics
    optimized = result.optimized_metrics
    return {
...
def show_sensitivity_plot(
    ...
    """Plot connected users and priority coverage for one changed parameter."""
...
def show_scenario_chart(df: pd.DataFrame) -> None:
    """Compare PSO gains across the available user-distribution scenarios."""
...
def show_reliability_chart(df: pd.DataFrame) -> None:
    """Show repeated-run spread using box plots."""
...
def _colored_boxplot(
    ...
    """Draw a box plot using the same colours as the placement methods."""
...
def show_dynamic_motion_chart(df: pd.DataFrame) -> None:
    """Plot how the metrics change while users move between time steps."""
...
def show_dynamic_uav_trails(df: pd.DataFrame) -> None:
    """Draw the PSO UAV positions from one time step to the next."""
    drone_ids = sorted(
        {
...
def show_weight_sensitivity_chart(df: pd.DataFrame) -> None:
    """Plot how changing the objective weights changes the trade-off."""
...
def show_weight_profile_radar(df: pd.DataFrame) -> None:
    """Show normalized trade-offs for each weight profile."""
    metrics = [
...
def run_dynamic_motion_analysis(
    ...
    """Move users slightly between time steps and re-run PSO."""
    simulation = DisasterNetworkSimulation(config)
    ...
    # Keep the interactive run short enough for the dashboard. Longer motion
    # experiments are better handled by the offline evidence script.
    ...
    for time_step in range(time_steps):
        ...
        # Simple random-walk motion: each user shifts a little before the next step.
...
def calculate_relay_ratio(graph, num_drones: int) -> float:
    """Calculate the proportion of possible UAV relay links that exist."""
...
def run_uav_sweep(
    ...
    """Run the same scenario with different numbers of UAVs."""
    rows = []
...
def run_area_sweep(
    ...
    """Run the same scenario with different square area sizes."""
    rows = []
...
def run_range_sweep(
    ...
    """Run the same scenario with different user-to-UAV ranges."""
    rows = []
...
def run_iteration_sweep(config: SimulationConfig, swarm_size: int) -> pd.DataFrame:
    """Run the same scenario with different PSO iteration counts."""
    rows = []
...
def run_scenario_comparison(
    ...
    """Run the same settings under each user-distribution scenario."""
    rows = []
...
def replace_config(config: SimulationConfig, **updates: object) -> SimulationConfig:
    """Copy a SimulationConfig while changing only selected fields."""
    values = {
