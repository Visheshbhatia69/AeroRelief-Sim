"""Metric calculations for comparing UAV placement methods."""
...
def evaluate_solution(
    ...
    """Evaluate one UAV placement against the selected scenario."""
...
def _connected_user_count(config: SimulationConfig, graph: nx.Graph) -> int:
    """Count users that are in a component with at least one UAV."""
...
def _coverage_percent(config: SimulationConfig, drones: np.ndarray, grid_size: int = 60) -> float:
    """Estimate covered map area using a regular grid of sample points."""
...
def _priority_coverage_percent(
    ...
    """Measure weighted coverage of emergency priority zones."""
...
def _ground_coverage_radius(config: SimulationConfig) -> float:
    """Convert communication range and UAV altitude into ground radius."""
...
def _average_path_length(graph: nx.Graph) -> float:
    """Use the largest component so isolated nodes do not break the calculation."""
