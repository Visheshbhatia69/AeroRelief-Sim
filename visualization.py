...
"""Create plots for the UAV disaster communication simulator."""
...
def plot_comparison(
    ...
    """Save a baseline vs K-Means vs PSO network plot."""
...
def plot_metric_charts(
    ...
    """Save main charts where higher values always mean better performance."""
...
def _relative_cohesion_scores(metrics_list: list[dict[str, float]]) -> list[float]:
    """Convert component counts into a higher-is-better cohesion score."""
...
def _priority_service_score(metrics: dict[str, float], cohesion_score: float) -> float:
    """Combine the most relevant higher-is-better metrics for a single view."""
    return float(
...
def plot_efficiency_chart(
    ...
    """Save movement-cost and improvement-per-distance charts."""
...
def plot_pso_convergence(score_history: list[float], output_path: Path) -> None:
    """Save a line chart showing PSO improvement over iterations."""
...
def plot_3d_deployment(
    ...
    """Save a 3D view with users on ground and UAVs at fixed altitude."""
...
def _bar_pair(
    ...
    """Draw one baseline-vs-PSO bar comparison."""
...
def _plot_network(
    ...
    """Draw one UAV network map on a Matplotlib axis."""
    axis.set_title(title)
...
def _plot_3d_network(
    ...
    """Draw one 3D deployment subplot."""
...
def _draw_3d_ground(axis: plt.Axes, config: SimulationConfig) -> None:
    """Draw a pale disaster-area ground plane."""
    vertices = [
        [
            (0.0, 0.0, 0.0),
...
def _draw_3d_priority_zones(axis: plt.Axes, priority_zones: list[PriorityZone]) -> None:
    """Draw priority-zone circles on the ground plane."""
...
def _draw_3d_obstacles(axis: plt.Axes, obstacles: list[Obstacle]) -> None:
    """Draw rectangular 3D obstacle blocks."""
...
def _draw_3d_coverage_circle(
    ...
    """Draw one UAV ground coverage footprint in the 3D plot."""
...
def _draw_priority_zones(axis: plt.Axes, priority_zones: list[PriorityZone]) -> None:
    """Draw hospital, shelter, rescue, and risk zones."""
    for zone in priority_zones:
        color = _priority_zone_color(zone.name)
...
def _draw_obstacles_2d(axis: plt.Axes, obstacles: list[Obstacle]) -> None:
    """Draw obstacle footprints on a 2D map."""
    for obstacle in obstacles:
...
def _priority_zone_color(zone_name: str) -> str:
    """Choose a readable priority-zone color for demo and OSM-backed zones."""
...
def _short_priority_label(zone_name: str) -> str:
    """Keep map labels compact enough for dashboard screenshots."""
...
def _draw_edges(axis: plt.Axes, graph: nx.Graph) -> None:
    """Draw user-to-UAV and UAV-to-UAV communication links."""
